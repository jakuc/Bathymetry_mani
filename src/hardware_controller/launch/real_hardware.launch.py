...
def _launch_setup(context, *args, **kwargs):
    ...
    mappings = {
        ...
        "jrt_min_range": LaunchConfiguration("jrt_min_range").perform(context),
        "jrt_max_range": LaunchConfiguration("jrt_max_range").perform(context),
        "jrt_timeout_ms": LaunchConfiguration("jrt_timeout_ms").perform(context),
        "jrt_recover_after_misses": LaunchConfiguration("jrt_recover_after_misses").perform(context),
    }
...
def generate_launch_description():
    return LaunchDescription([
        ...
        DeclareLaunchArgument("jrt_measure_mode", default_value="fast",
                              description="fast (~1,9 Hz) / slow (~0,3 Hz) / auto. Fast ma tę samą powtarzalność, a jest 6x szybszy"),
        DeclareLaunchArgument("jrt_period_ms", default_value="0",
                              description="Minimalny odstęp między strzałami; 0 = tak szybko, jak moduł nadąża"),
        DeclareLaunchArgument("jrt_min_range", default_value="0.03",
                              description="Filtr bzdur, nie specyfikacja - minimum tego egzemplarza niezmierzone"),
